...
describe('Cherry onboarding', () => {
  it('auto-opens only for a first empty workspace in the current session', () => {
    expect(
      shouldAutoOpenCherryOnboarding({ seen: false, hasWorkspace: true, taskCount: 0 }),
    ).toBe(true);
    expect(
      shouldAutoOpenCherryOnboarding({ seen: true, hasWorkspace: true, taskCount: 0 }),
    ).toBe(false);
    expect(
      shouldAutoOpenCherryOnboarding({ seen: false, hasWorkspace: true, taskCount: 1 }),
    ).toBe(false);
    expect(
      shouldAutoOpenCherryOnboarding({ seen: false, hasWorkspace: false, taskCount: 0 }),
    ).toBe(false);
  });
